...
class Main{
    public static void main(String[] args){
        ...
        // Make sure you have TreePrinter available in your project.
        TreePrinter<TreeNode> t = new TreePrinter<>(n -> "" + n.val, n -> n.left, n -> n.right);
...
class BSToperations{
    TreeNode insert(TreeNode root, int x){
        ...
        updateheight(root);
        return getbalancing(root);
    }

    // make this return int so assignments work correctly
    ...
    // safe when node is null
    int getBF(TreeNode node){
        if(node == null) return 0;
        int lh = (node.left == null ? 0 : node.left.h);
        int rh = (node.right == null ? 0 : node.right.h);
        return lh - rh;
    }

    TreeNode getbalancing(TreeNode root){
        ...
        // Right heavy (BF = -2)
        if(bf == -2){
            int rbf = getBF(root.right);

            // RR case: right-right (rotate left)
            if(rbf <= 0){
            ...
            // RL case: right-left (rotate right on root.right, then left on root)
            if(rbf > 0){
                TreeNode child = root.right;
                TreeNode newroot = child.left;

                // first: rotate right child (child) around newroot
                child.left = newroot.right;
                updateheight(child);

                // second: rotate root left around newroot
        ...
        // Left heavy (BF = 2)
        if(bf == 2){
            int lbf = getBF(root.left);

            // LL case: left-left (rotate right)
            if(lbf >= 0){
            ...
            // LR case: left-right (rotate left on root.left, then right on root)
            if(lbf < 0){
                TreeNode child = root.left;
                TreeNode newroot = child.right;

                // first: rotate left child (child) around newroot
                child.right = newroot.left;
                updateheight(child);

                // second: rotate root right around newroot
